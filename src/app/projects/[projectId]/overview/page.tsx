...
export default async function ProjectOverviewPage({ params }: { params: Promise<{ projectId: string }> }) {
  ...
  const [{ data: discovery }, { data: contextNode }, { data: repositoryMap }, { data: features }, { data: featureNodes }, { data: tasks }, { data: questions }, { data: humanTodos }] = await Promise.all([
    ...
    supabase.from("tasks").select("id, objective, state, branch_name, head_sha, developer_report, human_actions, updated_at, archived_at, features(name)").eq("project_id", projectId).order("updated_at", { ascending: false }).limit(20),
  ...
  return <div className="project-workspace"><ProjectNavigation projectId={project.id} projectName={project.name} projects={projects ?? []} automationState={project.automation_state as "running" | "frozen" | null} attentionCount={attentionCount} /><main className="workspace-main overview-main">
    ...
    <section className="overview-section"><div className="overview-section-heading"><div><p className="eyebrow">IMPLEMENTATION MAP</p><h2>What exists</h2></div><span className="map-placeholder-label">EVIDENCE TREE · NEXT</span></div><div className="implementation-map-placeholder"><div className="implementation-root">{project.name}<small>project</small></div><div className="implementation-branches">{features?.length ? features.map((feature) => <article key={feature.id}><i /><span className={feature.status}>{feature.status.replaceAll("_", " ")}</span><strong>{feature.name}</strong><small>Feature node</small></article>) : <p className="overview-empty">Feature nodes will appear after context approval.</p>}</div></div><p className="implementation-map-note">This placeholder will become the evidence tree: feature → delivered tasks → changed files → validation.</p></section>
