...
export async function GET(_request: Request, { params }: { params: Promise<{ projectId: string }> }) {
  ...
  const [{ data: project }, { data: tasks }, { data: features }, { data: questions }, { data: leases }, { data: events }, { data: humanTodos }] = await Promise.all([
    ...
    supabase.from("tasks").select("id, state, category, feature_id, archived_at, human_actions").eq("project_id", projectId).is("archived_at", null),
