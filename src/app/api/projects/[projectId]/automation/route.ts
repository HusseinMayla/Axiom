...
export async function GET(_request: Request, { params }: { params: Promise<{ projectId: string }> }) {
  ...
  const [{ data: project }, { data: tasks }, { data: leases }, { data: events }] = await Promise.all([
    ...
    supabase.from("tasks").select("state, category, archived_at").eq("project_id", projectId).is("archived_at", null),
  ...
  const hasGeneral = (tasks ?? []).some((task) => task.category === "general");
  return Response.json({ ...automationSnapshot({ state: project.automation_state, pauseReason: project.automation_pause_reason, cooldownUntil: project.automation_cooldown_until, lastActionAt: project.automation_last_action_at, leases: leases ?? [], hasReview: activeBranch, hasQueuedTask, canPropose: !hasGeneral }), events: events ?? [] });
}
  ...
  if (!isFrozen) {
    after(async () => {
      try {
        ...
        await Promise.all(results.map((result) => supabase.from("events").insert({ project_id: projectId, actor_type: "system", event_type: result.action === "idle" ? "automation_skipped" : "automation_claimed", payload: { ...result, trigger: "unfreeze" } })));
      } catch (cycleError) {
