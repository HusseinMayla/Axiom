...
export function DashboardActionCenter({ projectId, tasks, clarifications, featureSnapshots, humanTodos, planningFeatures, activity, automationState }: { projectId: string; tasks: DashboardTask[]; clarifications: DashboardClarification[]; featureSnapshots: FeatureSnapshot[]; humanTodos: HumanTodo[]; planningFeatures: PlanningFeature[]; activity: DashboardActivity[]; automationState: "running" | "frozen" | null }) {
  ...
  const items: Array<{ type: "review"; task: DashboardTask } | { type: "proposal"; task: DashboardTask } | { type: "retry"; task: DashboardTask } | { type: "clarification"; question: DashboardClarification } | { type: "prerequisite"; task: DashboardTask; action: DashboardTask["humanActions"][number] }> = [
    ...proposals.map((task) => ({ type: "proposal" as const, task })),
    ...failedTasks.map((task) => ({ type: "retry" as const, task })),
  ...
  const activeTask = tasks.find((task) => ["running", "pending_review", "waiting_for_human_approval"].includes(task.state)) ?? (manualControlsEnabled ? tasks.find((task) => task.state === "failed") ?? null : null);
  const runnableTasks = tasks.filter((task) => ["approved", "queued"].includes(task.state));
  ...
  async function request(key: string, url: string, body?: unknown) {
    ...
    setPending(null);
    if (!response.ok) { setMessage(payload.error ?? "Axiom could not apply that decision."); return; }
    if (url.endsWith("/plan-next")) setMessage(planningOutcomeMessage(payload));
  ...
  function renderItem(item: NonNullable<typeof primary>, primaryCard = false) {
    ...
    if (item.type === "proposal") { const { task } = item; const key = `proposal-${task.id}`; return <article className={primaryCard ? "decision-card primary-proposal" : "decision-card"} key={key}><div className="decision-card-header"><span className="decision-kind blue">TASK PROPOSAL</span><span className="decision-feature">{task.featureName}</span></div><h2>{task.objective}</h2><p>{task.humanSummary}</p><div className="decision-actions"><button className="button" disabled={pending === key} onClick={() => updateTask(key, task.id, { approve: true })}>{pending === key ? "Approving…" : "Approve task"}</button></div></article>; }
    if (item.type === "retry") { const { task } = item; const key = `retry-${task.id}`; return <article className={primaryCard ? "decision-card primary-prerequisite" : "decision-card"} key={key}><div className="decision-card-header"><span className="decision-kind amber">RECOVERY REQUIRED</span><span className="decision-feature">{task.featureName}</span></div><h2>{task.objective}</h2><p>{task.reviewFeedback ?? failureReason(task) ?? "This task did not complete safely. Return it to the execution queue before automatic flow can try it again."}</p><ExecutionDetails task={task} /><div className="decision-actions"><button className="button" disabled={pending === key} onClick={() => updateTask(key, task.id, { resetExecution: true })}>{pending === key ? "Returning…" : "Return to queue"}</button></div></article>; }
  ...
  return <>
    ...
    <ExecutionQueue projectId={projectId} tasks={runnableTasks} features={planningFeatures} hasActiveTask={Boolean(activeTask)} manualControlsEnabled={manualControlsEnabled} pending={pending} request={request} />
...
function ActiveTask({ projectId, task, feedback, setFeedback, pending, updateTask, request, manualControlsEnabled }: { projectId: string; task: DashboardTask | null; feedback: Record<string, string>; setFeedback: (value: Record<string, string>) => void; pending: string | null; updateTask: (key: string, taskId: string, body: Record<string, unknown>) => Promise<void>; request: (key: string, url: string, body?: unknown) => Promise<void>; manualControlsEnabled: boolean }) {
...
function TaskProposalComposer({ projectId, features, pending, request }: { projectId: string; features: PlanningFeature[]; pending: string | null; request: (key: string, url: string, body?: unknown) => Promise<void> }) {
  ...
  return <details className="task-proposal-composer"><summary>Propose task</summary><div><label>Scope<select value={category} onChange={(event) => setCategory(event.target.value as "general" | "feature")}><option value="general">General project work</option><option value="feature" disabled={!features.length}>Feature work</option></select></label>{category === "feature" ? <label>Feature<select value={featureId} onChange={(event) => setFeatureId(event.target.value)}>{features.map((feature) => <option value={feature.id} key={feature.id}>{feature.name}</option>)}</select></label> : null}<label>What should Axiom plan?<textarea value={recommendation} onChange={(event) => setRecommendation(event.target.value)} placeholder="Describe the bounded outcome you want Axiom to propose." /></label><button className="button" disabled={!valid || pending === "propose"} onClick={() => request("propose", `/api/projects/${projectId}/plan-next`, { request: { recommendation: recommendation.trim(), category, ...(category === "feature" ? { featureId } : {}) } })}>{pending === "propose" ? "Proposing…" : "Ask Axiom to propose"}</button></div></details>;
}
...
function SystemStatus({ tasks, clarifications }: { tasks: DashboardTask[]; clarifications: DashboardClarification[] }) {
  ...
  const status = running ? { label: "Executing task", detail: running.objective, active: true, tone: "green" } : reviewing ? { label: "Reviewing task", detail: reviewing.objective, active: true, tone: "blue" } : waiting ? { label: "Waiting for approval", detail: waiting.objective, active: false, tone: "amber" } : clarifications.length > 0 ? { label: "Checking context", detail: "A clarification needs your input.", active: true, tone: "blue" } : { label: "Idle", detail: "No harness action is currently running.", active: false, tone: "cyan" };
